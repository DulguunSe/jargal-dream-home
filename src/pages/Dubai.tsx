...
const Dubai = () => {
  ...
  const dubaiProps = useMemo(() => properties.filter((p) => p.is_dubai), [properties]);

  const filtered = useMemo(() => {
    return dubaiProps.filter((p) => {
      if (location && p.location !== location) return false;
      if (type && p.type !== type) return false;
      return true;
    });
  }, [dubaiProps, location, type]);
    ...
    <Layout>
      <section className="section-padding">
        <div className="container-wide">
          <div className="mb-10">
            <p className="text-accent font-semibold text-sm uppercase tracking-wider">{t("dubai.label")}</p>
            <h1 className="font-display text-3xl sm:text-4xl font-bold text-foreground mt-1">{t("dubai.title")}</h1>
          </div>
          ...
          {dubaiProps.length > 0 && (
            <div className="flex flex-col sm:flex-row gap-3 mb-8 p-4 bg-secondary rounded-lg">
              <select value={location} onChange={(e) => setLocation(e.target.value)} className={selectClass + " flex-1"}>
                ...
                {[...new Set(dubaiProps.map((p) => p.location))].map((loc) => (
              ...
              <select value={type} onChange={(e) => setType(e.target.value)} className={selectClass + " flex-1"}>
                ...
                {[...new Set(dubaiProps.map((p) => p.type))].map((t) => (
          ...
          {isLoading ? (
            ...
            <p className="text-center text-muted-foreground py-16">{t("dubai.noMatch")}</p>
          ) : (
